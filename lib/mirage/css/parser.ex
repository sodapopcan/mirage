...
defmodule Mirage.CSS.Parser do
  @moduledoc false

  alias Mirage.CSS.{Combinator, Element}
  alias Mirage.CSS.Element.{Attribute, Namespace, PseudoClass, Tag}
  ...
  defp parse_element([~c":" | toks], element) do
    ...
    element = %{element | selectors: [selector | element.selectors]}
    validate_pseudo_class!(selector)
    parse_element(toks, element)
  end
  ...
  defp parse_element(toks, element) do
    raise "CSS selector parse error: unsupported tokens #{inspect(toks)} in #{inspect(element)}"
  end
  ...
  defp parse_attribute(toks) do
    raise "CSS selector parse error: unsupported attribute tokens #{inspect(toks)}"
  end
  ...
  defp pseudo_class_selector(type, args) do
    case type do
      ~c"first-child" -> %PseudoClass.FirstChild{args: args}
      ~c"first-of-type" -> %PseudoClass.FirstOfType{args: args}
      ~c"last-child" -> %PseudoClass.LastChild{args: args}
      ~c"last-of-type" -> %PseudoClass.LastOfType{args: args}
      ~c"not" -> %PseudoClass.Not{args: args}
      ~c"nth-child" -> %PseudoClass.NthChild{args: args}
      ~c"nth-last-child" -> %PseudoClass.NthLastChild{args: args}
      ~c"nth-last-of-type" -> %PseudoClass.NthLastOfType{args: args}
      ~c"nth-of-type" -> %PseudoClass.NthOfType{args: args}
      _ -> raise "CSS selector parse error: pseudo class \"#{type}\" not supported"
    end
  end

  defp validate_pseudo_class!(%{args: args} = selector)
       when is_struct(selector, PseudoClass.NthChild)
       when is_struct(selector, PseudoClass.NthLastChild)
       when is_struct(selector, PseudoClass.NthOfType)
       when is_struct(selector, PseudoClass.NthLastOfType) do
    valid? =
      match?(["even"], args) or match?(["odd"], args) or
        match?([n] when is_integer(n), args) or
        match?([a, b] when is_integer(a) and is_integer(b), args)

    unless valid? do
      raise "CSS selector parse error: #{inspect(selector.__struct__)} has invalid arguments: #{inspect(args)}"
    end
  end

  defp validate_pseudo_class!(_), do: :ok
end
